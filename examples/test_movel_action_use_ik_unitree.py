...
使用测试关节角，先 MoveJ 到目标关节角度，
再读取当前位姿，计算 Z 轴偏移后的目标位姿，
...
def build_offset_pose(source_pose, z_offset):
...
def main():
    print("=" * 70)
    print("直线运动 Action 测试 - Unitree")
    print("步骤:")
    print("  1. MoveJ 移动到目标关节角度")
    print("  2. 读取当前位姿")
    print("  3. 计算 Z 轴偏移后的目标位姿")
    ...
    try:
        ...
        print("\n[6] 选择测试模式:")
        print("1. 左臂 - MoveJ 到目标关节 -> action 直线 Z 轴下移 0.05m")
        print("2. 右臂 - MoveJ 到目标关节 -> action 直线 Z 轴上移 0.05m")
        ...
        duration = 3.0
        if choice == "1":
            left_target_pose = build_offset_pose(client.get_current_pose("left"), z_offset=-0.05)
            print(
            ...
            right_target_pose = build_offset_pose(client.get_current_pose("right"), z_offset=0.05)
            print(
