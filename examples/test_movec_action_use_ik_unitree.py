...
class CircleTrajectoryActionClient(Node):
    ...
    def send_dual_movej_command(self, left_joints, right_joints, duration=4.0):
        ...
        self.get_logger().error(
            "MoveJ 失败" + (f"，消息: {result.message}" if result else "")
        )
        return False

    def movej_feedback_callback(self, feedback):
        print(
            f"    MoveJ反馈: 进度 {feedback.progress * 100.0:.1f}%, "
...
def main():
    ...
    try:
        ...
        print("\n选择测试模式:")
        print("1. 左臂  - MoveJ 到起点 -> 圆弧 action (三点法)")
        print("2. 右臂  - MoveJ 到起点 -> 圆弧 action (三点法)")
        print("3. 双臂  - 同时 MoveJ 到起点 -> 单次圆弧 action 同时执行 (三点法)")
        print("4. 左臂  - MoveJ 到起点 -> 圆弧 action (参数法)")
        print("5. 右臂  - MoveJ 到起点 -> 圆弧 action (参数法)")
        print("6. 双臂  - 同时 MoveJ 到起点 -> 单次圆弧 action 同时执行 (参数法)")
        ...
        if choice == "1":
            ...
            left_center = Point(x=LEFT_PARAMETRIC_CENTER[0], y=LEFT_PARAMETRIC_CENTER[1], z=LEFT_PARAMETRIC_CENTER[2])
            left_axis = Vector3(x=LEFT_PARAMETRIC_AXIS[0], y=LEFT_PARAMETRIC_AXIS[1], z=LEFT_PARAMETRIC_AXIS[2])
            right_center = Point(x=RIGHT_PARAMETRIC_CENTER[0], y=RIGHT_PARAMETRIC_CENTER[1], z=RIGHT_PARAMETRIC_CENTER[2])
            right_axis = Vector3(x=RIGHT_PARAMETRIC_AXIS[0], y=RIGHT_PARAMETRIC_AXIS[1], z=RIGHT_PARAMETRIC_AXIS[2])
            print(
