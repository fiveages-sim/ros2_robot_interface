...
异常或中断时自动停止录制，并在 ``record_data/target_stamped_*`` 中生成 CSV 与图表。

运行（从 fa-py-libraries 根目录）：
    .venv/bin/python ros2_robot_interface/examples/test/16_joint_trajectory_compare/move_left_up_target_stamped.py
...
def main() -> int:
    ...
    try:
        ...
        print("    启动关节接口录制（--plot）...")
        recorder = start_recorder("target_stamped")
