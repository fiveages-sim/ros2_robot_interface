    ...
    try:
        while True:
            ...
            if left_gripper_available:
                try:
                    # 发送位置命令
                    interface.left_gripper_handler.send_joint_positions(test_position)
                    print(f"[{i:3d}s] [左夹爪] 发送目标位置: {test_position:.2f}")
                    ...
                    if joint_state:
                        ...
                        if current_pos is not None:
                            ...
                            if arrived:
                                print(f"[{i:3d}s] [左夹爪] ✓ 已到达目标位置")
                            else:
                                print(f"[{i:3d}s] [左夹爪] ⚠ 未到达目标位置")
                        else:
                            print(f"[{i:3d}s] [左夹爪] ⚠ 无法获取当前位置")
                except Exception as e:
                    print(f"[{i:3d}s] ✗ 左夹爪控制失败: {e}")
            
            # 右夹爪：发送位置命令并检测到达（仅双臂模式）
            if right_gripper_available:
                try:
                    # 发送位置命令
                    interface.right_gripper_handler.send_joint_positions(test_position)
                    print(f"[{i:3d}s] [右夹爪] 发送目标位置: {test_position:.2f}")
                    ...
                    if joint_state:
                        ...
                        if current_pos is not None:
                            ...
                            # check_arrival 方法内部已经打印了详细信息，这里只打印简要结果
                            if arrived:
                                print(f"[{i:3d}s] [右夹爪] ✓ 已到达目标位置")
                            else:
                                print(f"[{i:3d}s] [右夹爪] ⚠ 未到达目标位置")
                        else:
                            print(f"[{i:3d}s] [右夹爪] ⚠ 无法获取当前位置")
                except Exception as e:
                    print(f"[{i:3d}s] ✗ 右夹爪控制失败: {e}")
