...
class Waypoint:
    name: str
    body_joints_rad: list[float]
    left_xyz: list[float]
    left_rpy: list[float]
    rpy_unit: str  # "deg" or "rad"
...
def wait_body_arrival(interface: ROS2RobotInterface, timeout: float) -> bool:
    ...
    while time.time() - start_time < timeout:
        try:
            if interface.check_arrive("body", threshold=BODY_THRESHOLD):
                elapsed = time.time() - start_time
...
def build_sequence(x: float, y: float, x1: float, y1: float) -> list[Waypoint]:
    ...
    return [home, pick_up, pick, pick_up, home, place_up, place, place_up, home]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--y", type=float, default=0.2, help="抓取点 y（米）")
    parser.add_argument("--x1", type=float, default=0.4, help="放置点 x1（米）")
